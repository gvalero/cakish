...
export function buildInquirySummary(input: InquirySummaryInput): string {
  const lines = [
    ...
    `Filling: ${input.fillingLabel}${input.fillingSurcharge > 0 ? ` (+EUR ${input.fillingSurcharge.toFixed(2)})` : " (included)"}`,
  ];
  if (input.hasFinishOptions) {
    lines.push(
      `Finish: ${input.finishLabel}${input.finishSurcharge > 0 ? ` (+EUR ${input.finishSurcharge.toFixed(2)})` : " (included)"}`
    );
  }
  if (input.topperMessage.trim()) {
    lines.push(`Custom topper message: "${input.topperMessage.trim()}" (+EUR ${input.topperPrice.toFixed(2)})`);
  }
