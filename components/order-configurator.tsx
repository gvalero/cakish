...
import { calculatePrice, buildInquirySummary } from "@/lib/pricing";
...
export function OrderConfigurator() {
  ...
  // Sync from ?product= URL param after hydration. This runs client-side only,
  // matching the server-rendered default (products[0]) during hydration and then
  // updating to the URL-specified product — the correct pattern for static exports.
  useEffect(() => {
  ...
  // Run once on mount — intentional single-run initialisation from URL.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const showFinishOptions = product.hasFinishOptions;
  ...
  // Precompute step numbers so they stay stable across renders and account
  // for optional sections (finish, topper) being conditionally shown.
  const steps = useMemo(() => {
    let n = 1;
    return {
      product: n++,
      size: n++,
      filling: n++,
      finish: showFinishOptions ? n++ : 0,
      topper: product.hasTopper ? n++ : 0,
    };
  }, [showFinishOptions, product.hasTopper]);

  const priceDisplay = useMemo(() => calculatePrice({
    basePrice: selectedSize.price,
    fillingSurcharge: selectedFillingOption.surcharge,
    finishSurcharge: selectedFinishOption.surcharge,
    hasFinishOptions: showFinishOptions,
    topperPrice: product.topperPrice,
    topperMessage,
    quantity,
  }), [selectedSize.price, selectedFillingOption.surcharge, selectedFinishOption.surcharge, showFinishOptions, product.topperPrice, topperMessage, quantity]);

  const inquirySummary = useMemo(() => buildInquirySummary({
    productName: product.name,
    sizeLabel: selectedSize.label,
    sizeDiameter: selectedSize.diameter,
    sizeServes: selectedSize.serves,
    fillingLabel: selectedFillingOption.label,
    fillingSurcharge: selectedFillingOption.surcharge,
    hasFinishOptions: showFinishOptions,
    finishLabel: selectedFinishOption.label,
    finishSurcharge: selectedFinishOption.surcharge,
    topperMessage,
    topperPrice: product.topperPrice,
    quantity,
    total: priceDisplay.total,
    hasPrice: priceDisplay.hasPrice,
  ...
  const handleWhatsApp = () => {
    ...
    const base = siteContent.whatsapp
      ? `https://wa.me/${siteContent.whatsapp}`
      : "https://wa.me/";
    window.open(`${base}?text=${text}`);
  };
      ...
      <div className="space-y-10">
        ...
        <fieldset>
          <legend className="mb-5">
            <StepLabel step={steps.product} label="Choose your pavlova" />
          </legend>
        ...
        <fieldset>
          <legend className="mb-5">
            <StepLabel step={steps.size} label="Select your size" />
          </legend>
        ...
        <fieldset>
          <legend className="mb-5">
            <StepLabel step={steps.filling} label="Choose your filling" />
          </legend>
        ...
        {showFinishOptions && (
          <>
            ...
            <fieldset>
              <legend className="mb-5">
                <StepLabel step={steps.finish} label="Choose your finish" />
        ...
        {product.hasTopper && (
          <fieldset>
            <legend className="mb-5">
              <StepLabel step={steps.topper} label="Custom topper message" />
              <p className="mt-1 text-xs text-[color:var(--muted-copy)]">
      ...
      >
        ...
        <div className="mt-6 space-y-3 border-t border-[color:var(--line)] pt-5 text-sm text-[color:var(--body-copy)]">
          <div className="flex justify-between">
            <span>Size base</span>
            <span>EUR {selectedSize.price.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
      ...
      >
        ...
        >
          ...
          <div className="mt-8 space-y-4">
            ...
            <div className="space-y-4 border border-[color:var(--line)] bg-[color:var(--soft-cream)] p-4 text-sm leading-6 text-[color:var(--body-copy)]">
              <p className="font-semibold uppercase tracking-[0.18em] text-[color:var(--soft-gold)] text-xs">
                How to complete your order
              </p>
              <p>
                Online payment is coming soon. For now, please use the enquiry options below to send your order to us — we will confirm your collection date and arrange payment.
              </p>
              <div className="flex flex-wrap gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => { setDrawerOpen(false); handleWhatsApp(); }}
                  className="flex items-center gap-1.5 rounded-sm border border-[color:var(--line)] bg-white px-3 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.1em] text-[color:var(--muted-copy)] transition hover:border-[#25D366] hover:text-[#25D366]"
                  aria-label="Send order via WhatsApp"
                >
                  <WhatsAppIcon />
                  WhatsApp
                ...
                <button
                  type="button"
                  onClick={() => { setDrawerOpen(false); handleEmail(); }}
                  className="flex items-center gap-1.5 rounded-sm border border-[color:var(--line)] bg-white px-3 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.1em] text-[color:var(--muted-copy)] transition hover:border-[color:var(--deep-charcoal)] hover:text-[color:var(--deep-charcoal)]"
                  aria-label="Send order via email"
                >
                  Email
                </button>
